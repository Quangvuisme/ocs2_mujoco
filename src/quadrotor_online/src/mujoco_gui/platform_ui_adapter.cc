...
namespace mujoco
{
  ...
  void PlatformUIAdapter::OnKey(int key, int scancode, int act)
  {
    ...
    if (act == GLFW_PRESS)
    {
      command_gripped = (key == GLFW_KEY_KP_4);
      command_ungripped = (key == GLFW_KEY_KP_6);
      if (key==GLFW_KEY_KP_1)
        publish_position = 43; // clock
      if (key==GLFW_KEY_KP_2)
        publish_position = 39; // cartridge
      if (key==GLFW_KEY_KP_3)
        publish_position = 41; // dino
      if (key==GLFW_KEY_KP_5)
        publish_position = 47; // basket
      if (key==GLFW_KEY_KP_8)
        publish_position = 51; // cartridge 2
      if (key==GLFW_KEY_KP_9)
        publish_position = 15; // camera_body
      if (key==GLFW_KEY_KP_ADD){
        is_publishing = !is_publishing;
        std::cout << "Publishing Images = " << is_publishing << std::endl;
      }
    }
    else
    {
      command_gripped = false;
      command_ungripped = false;
    }
