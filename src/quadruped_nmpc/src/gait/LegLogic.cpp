#include "quadruped_nmpc/gait/LegLogic.h"
...
namespace {
    ...
    bool hasStartTime(const quadruped_nmpc::ContactTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const quadruped_nmpc::ContactTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool hasStartTime(const quadruped_nmpc::SwingTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const quadruped_nmpc::SwingTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool startsWithSwingPhase(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithContactPhase(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return !startsWithSwingPhase(timings);
    }

    bool endsWithSwingPhase(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithContactPhase(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return !endsWithSwingPhase(timings);
    }

    bool startsWithContactPhase(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithSwingPhase(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return !startsWithContactPhase(timings);
    }

    bool endsWithContactPhase(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithSwingPhase(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return !endsWithContactPhase(timings);
    }

    bool touchesDownAtLeastOnce(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const quadruped_nmpc::ContactTiming &timing) {
            return hasStartTime(timing);
        });
    }

    bool liftsOffAtLeastOnce(const std::vector<quadruped_nmpc::ContactTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool touchesDownAtLeastOnce(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool liftsOffAtLeastOnce(const std::vector<quadruped_nmpc::SwingTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const quadruped_nmpc::SwingTiming &timing) {
            return hasStartTime(timing);
        });
    }
} // anonymous namespace
