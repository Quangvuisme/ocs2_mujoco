#include <anymal_walking_online/gait/GaitSchedule.h>
...
namespace anymal_walking_online {
    ...
    void GaitSchedule::insertModeSequenceTemplate(const ModeSequenceTemplate &modeSequenceTemplate,
                                                  ocs2::scalar_t startTime, ocs2::scalar_t finalTime) {
        ...
        ocs2::scalar_t phaseTransitionStanceTime = phaseTransitionStanceTime_;
    ...
    ocs2::ModeSchedule GaitSchedule::getModeSchedule(ocs2::scalar_t lowerBoundTime, ocs2::scalar_t upperBoundTime) {
        auto &eventTimes = modeSchedule_.eventTimes;
        auto &modeSequence = modeSchedule_.modeSequence;
    ...
    void GaitSchedule::tileModeSequenceTemplate(ocs2::scalar_t startTime, ocs2::scalar_t finalTime) {
        auto &eventTimes = modeSchedule_.eventTimes;
        auto &modeSequence = modeSchedule_.modeSequence;
        ...
        while (eventTimes.back() < finalTime) {
            for (size_t i = 0; i < templateModeSequence.size(); i++) {
                ...
                ocs2::scalar_t deltaTime = templateTimes[i + 1] - templateTimes[i];
