...
namespace
{
  ...
  //   int nisland = mjMIN(d->solver_niter[0], mjNISLAND);
   ...
   void UpdateProfiler(mj::Simulate *sim, const mjModel *m, const mjData *d)
{
  ...
  // update Constraint figure (1 island version)
  int npoints = mjMIN(d->solver_niter[0], mjMAXLINEPNT);  // total iterations
  for (int i = 0; i < kConstraintNum; i++)
    sim->figconstraint.linepnt[i] = npoints;

  for (int i = 0; i < npoints; i++)
  {
    const mjSolverStat *stat = d->solver + i;
    for (int j = 0; j < kConstraintNum; j++)
    {
      sim->figconstraint.linedata[j][2 * i] = i;
    }
    sim->figconstraint.linedata[0][2 * i + 1] = d->nefc;
    sim->figconstraint.linedata[1][2 * i + 1] = stat->nactive;
    sim->figconstraint.linedata[2][2 * i + 1] = stat->nchange;
    sim->figconstraint.linedata[3][2 * i + 1] = stat->neval;
    sim->figconstraint.linedata[4][2 * i + 1] = stat->nupdate;
  }

  // update Cost figure
  for (int i = 0; i < kCostNum; i++)
    sim->figcost.linepnt[i] = npoints;

  for (int i = 0; i < npoints; i++)
  {
    const mjSolverStat *stat = d->solver + i;
    for (int j = 0; j < kCostNum; j++)
      sim->figcost.linedata[j][2 * i] = i;

    sim->figcost.linedata[0][2 * i + 1] = mju_log10(mju_max(mjMINVAL, stat->improvement));
    sim->figcost.linedata[1][2 * i + 1] = mju_log10(mju_max(mjMINVAL, stat->gradient));
    sim->figcost.linedata[2][2 * i + 1] = mju_log10(mju_max(mjMINVAL, stat->lineslope));
  }
  ...
  // update figsize (no solver_nnz anymore, approximate with nefc)
  float sdata[6] = {
      static_cast<float>(m->nv),
      static_cast<float>(m->nbody),
      static_cast<float>(d->nefc),
      static_cast<float>(mju_sqrt((mjtNum)d->nefc)),  // approximate nnz
      static_cast<float>(d->ncon),
      static_cast<float>(d->solver_niter[0])};
  ...
  void UiEvent(mjuiState *state)
  {
        ...
        state->type == mjEVENT_KEY)
    {
      ...
      // MPC target section
      else if (it && it->sectionid == SECT_TARGET)
      {
        // Target selection changed
        if (sim->target_index != sim->target_index_prev)
        {
          sim->target_index_prev = sim->target_index;
          sim->target_changed.store(true);
          std::cout << "[UI] Target changed to: " << (sim->target_index + 1) << std::endl;
        }
      }
...
#undef X
  ...
  void Simulate::RenderLoop()
  {
    ...
    mjui_add(&this->ui0, this->def_target);
    this->ui0.sect[0].state = 1;
    this->ui0.sect[1].state = 1;
    this->ui0.sect[2].state = 1;
    this->ui0.sect[3].state = 1;  // Target section open by default
    ...
    while (!this->platform_ui->ShouldCloseWindow() && !this->exitrequest.load())
    {
      ...
      if (this->platform_ui->command_gripped){
        d_->ctrl[23] = 255;
        this->platform_ui->command_gripped = false;
      }
      if (this->platform_ui->command_ungripped){
        d_->ctrl[23] = 0;
        this->platform_ui->command_ungripped = false;
      }
      if (this->platform_ui->publish_position>0){
        auto body_id = this->platform_ui->publish_position;
        int pelvis_id = 14;
        Eigen::Matrix<double,3,1> obj_xpos = Eigen::Matrix<double,3,1>();
        obj_xpos <<  d_->xpos[3*body_id], d_->xpos[3*body_id+1], d_->xpos[3*body_id+2];
        std::cout << "Absolute World Coordinates : object " << body_id << " xpos = " << obj_xpos(0) << " , " << obj_xpos(1) << " , " << obj_xpos(2) << std::endl;

        Eigen::Matrix<double,3,1> pelvis_xpos = Eigen::Matrix<double,3,1>();
        pelvis_xpos <<  d_->xpos[3*pelvis_id], d_->xpos[3*pelvis_id+1], d_->xpos[3*pelvis_id+2];
        obj_xpos = obj_xpos - pelvis_xpos;
        //std::cout << "REL : object " << body_id << " xpos = " << obj_xpos(0) << " , " << obj_xpos(1) << " , " << obj_xpos(2) << std::endl;

        mjtNum relative_pos[3];
        mjtNum inverse_quat[4];
        mjtNum rotated[3];
        mjtNum relative_quat[4];

        mju_sub(relative_pos, &d_->xpos[3*body_id],&d_->xpos[3*pelvis_id], 3);
        mju_negQuat(inverse_quat,&d_->xquat[4*pelvis_id]);
        mju_rotVecQuat(rotated,relative_pos,inverse_quat);
        mju_mulQuat(relative_quat, inverse_quat, &d_->xquat[4*body_id]);

        std::cout << "Relative Transformations : object " << body_id << " xpos = " << rotated[0] << " , " << rotated[1] << " , " << rotated[2] << std::endl;
        std::cout << "Relative Transformations : object " << body_id << " xquat = " << relative_quat[0] << " , " << relative_quat[1] << " , " << relative_quat[2] << "," << relative_quat[3] << std::endl;

        // if (bridge_){
        //   // bridge_->PublishStartPoint(obj_xpos(0),obj_xpos(1),obj_xpos(2));
        //   auto basketPos = bridge_->GetRelativePos("basket_model","pelvis_link");
        //   // bridge_->PublishEndPoint(basketPos(0),basketPos(1),basketPos(2));
        //   auto relative = Eigen::Matrix<double,3,1> ();
        //   relative << rotated[0], rotated[1], rotated[2];
        //   std::cout << "REL_CAL : object " << body_id << " xpos = " << rotated[0] << " , " << rotated[1] << " , " << rotated[2] << std::endl;
        //   std::cout << "REL_CAL : object " << body_id << " xquat = " << relative_quat[0] << " , " << relative_quat[1] << " , " << relative_quat[2] << "," << relative_quat[3] << std::endl;
        //   std::cout << "REL_CAL : basket is at xpos = " << basketPos(0) << " , " << basketPos(1) << " , " << basketPos(2) << std::endl;
        //   bridge_->PublishSelectPoints(relative,basketPos);
        // }

        this->platform_ui->publish_position = 0;
      }

      // render while simulation is running
      this->Render();

      // Camera publishing disabled for quadrotor_online
      // #ifdef USE_ROS2
      // this->publishCamera();
      // #endif
  ...
  #ifdef USE_ROS2
  void Simulate::setRosNode(std::shared_ptr<CameraNode> node){
    node_ = node;
  }

  void Simulate::publishCamera(){
    auto elapsed = Milliseconds(mj::Simulate::Clock::now() - last_published);
    // std::cout << elapsed.count() << " ms passed." << std::endl;
    if (elapsed.count()<100)
      return;

    last_published = mj::Simulate::Clock::now();

    if (node_){
      mjrRect viewport =  mjr_maxViewport(&this->platform_ui->mjr_context());
      int W = viewport.width;
      int H = viewport.height;
      node_->checkResourcesAllocation(W,H,m_->cam_fovy[0]);
      mjr_readPixels(node_->getRgbBuffer(), node_->getDepthBuffer(), viewport, &this->platform_ui->mjr_context());
      if (this->platform_ui->is_publishing)
        node_->publishCameraFrame();

      // if (bridge_->camera_pixels != W*H){
      //   bridge_->AllocateBuffer(W*H);
      //   bridge_->CalculateIntrinsics(W,H,m_->cam_fovy[0]);
      //   std::cout<< "Extent = " << m_->stat.extent << " , znear = " << m_->vis.map.znear << " , zfar = " << m_->vis.map.zfar << " . Read Depth Map = " << this->platform_ui->mjr_context().readDepthMap << " Width = " << W << " Height = " << H << std::endl;
      // }
      // mjr_readPixels(bridge_->rgb, bridge_->depth, viewport, &this->platform_ui->mjr_context());
      // bridge_->PublishCamera(W,H);
    }
  }
  #endif
