...
// #include "../ros/CameraNode.hpp"  // Disabled for quadrotor_online
...
namespace mujoco
{
  ...
  public:
    ...
    #ifdef USE_ROS2
    void setRosNode(std::shared_ptr<CameraNode> node);
    #endif
    void publishCamera();
    ...
    std::chrono::time_point<Clock> last_published = Clock::now();
    ...
    // MPC target selection (for quadrotor control)
    int target_index = 0;           // Current target index (0-3)
    int target_index_prev = -1;     // Previous target index for change detection
    std::atomic<bool> target_changed{false};  // Flag to signal target change
    ...
    // MPC target section of UI
    const mjuiDef def_target[4] = {
        {mjITEM_SECTION, "MPC Target", mjPRESERVE, nullptr, "AT"},
        {mjITEM_SELECT, "Target", 2, &this->target_index, "Target 1 [0.5,0.5,1.5]\nTarget 2 [1,0,1.5]\nTarget 3 [0,1,2]\nTarget 4 [-0.5,-0.5,0.8]"},
        {mjITEM_STATIC, "Status", 2, nullptr, "Ready"},
        {mjITEM_END}};
