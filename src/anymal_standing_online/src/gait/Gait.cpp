#include <anymal_standing_online/gait/Gait.h>
...
namespace anymal_standing_online {
    bool isValidGait(const Gait &gait) {
        ...
        validGait &= std::all_of(gait.eventPhases.begin(), gait.eventPhases.end(),
                                 [](ocs2::scalar_t phase) { return 0.0 < phase && phase < 1.0; });
    ...
    bool isValidPhase(ocs2::scalar_t phase) {
        return phase >= 0.0 && phase < 1.0;
    }


    ocs2::scalar_t wrapPhase(ocs2::scalar_t phase) {
    ...
    int getModeIndexFromPhase(ocs2::scalar_t phase, const Gait &gait) {
    ...
    size_t getModeFromPhase(ocs2::scalar_t phase, const Gait &gait) {
        assert(isValidPhase(phase));
        assert(isValidGait(gait));
    ...
    ocs2::scalar_t timeLeftInGait(ocs2::scalar_t phase, const Gait &gait) {
        assert(isValidPhase(phase));
        assert(isValidGait(gait));
    ...
    ocs2::scalar_t timeLeftInMode(ocs2::scalar_t phase, const Gait &gait) {
        assert(isValidPhase(phase));
        assert(isValidGait(gait));
