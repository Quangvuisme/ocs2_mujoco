...
#include "quadrotor_offline/QuadrotorDynamics.h"
...
QuadrotorInterface::QuadrotorInterface(const std::string& taskFile,
  ...
  // Dynamics
  auto quadrotorParameters = loadSettings(taskFile, "QuadrotorParameters", verbose);
  problem_.dynamicsPtr = std::make_unique<QuadrotorDynamics>(quadrotorParameters);
