#include "anymal_standing_online/gait/LegLogic.h"
...
namespace {
    ...
    bool hasStartTime(const anymal_standing_online::ContactTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const anymal_standing_online::ContactTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool hasStartTime(const anymal_standing_online::SwingTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const anymal_standing_online::SwingTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool startsWithSwingPhase(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithContactPhase(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return !startsWithSwingPhase(timings);
    }

    bool endsWithSwingPhase(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithContactPhase(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return !endsWithSwingPhase(timings);
    }

    bool startsWithContactPhase(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithSwingPhase(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return !startsWithContactPhase(timings);
    }

    bool endsWithContactPhase(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithSwingPhase(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return !endsWithContactPhase(timings);
    }

    bool touchesDownAtLeastOnce(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const anymal_standing_online::ContactTiming &timing) {
            return hasStartTime(timing);
        });
    }

    bool liftsOffAtLeastOnce(const std::vector<anymal_standing_online::ContactTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool touchesDownAtLeastOnce(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool liftsOffAtLeastOnce(const std::vector<anymal_standing_online::SwingTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const anymal_standing_online::SwingTiming &timing) {
            return hasStartTime(timing);
        });
    }
} // anonymous namespace
