...
XboxController::XboxController() {
    ...
    std::vector<std::string> controllerNames = {
        ...
        "Xbox 360 Wireless Receiver"           // Xbox 360 wireless
    };
    ...
    if (devicePath.empty()) {
        ...
        std::cerr << "[Xbox] Connect Xbox controller and restart for joystick support." << std::endl;
        return;
    }
...
void XboxController::runThread() {
    ...
    while (running_) {
        ...
        if (ev.type == EV_KEY) {
            switch (ev.code) {
                case BTN_A:  // A button - PASSIVE mode
                    if (ev.value == 1) modeChangeRequest_.store(0);
                    break;
                case BTN_B:  // B button - PD_CONTROL mode
                    if (ev.value == 1) modeChangeRequest_.store(1);
                    break;
                case BTN_X:  // X button - STANDING mode (MPC)
                    if (ev.value == 1) modeChangeRequest_.store(2);
                    break;
                case BTN_Y:  // Y button - WALKING mode (MPC + Gait)
                    if (ev.value == 1) modeChangeRequest_.store(3);
                    break;
                case BTN_START:  // Start button - Toggle MPC
                    if (ev.value == 1) startPressed_.store(true);
                    break;
                case BTN_SELECT:  // Back/Select button - Emergency stop
                    if (ev.value == 1) backPressed_.store(true);
                    break;
                case BTN_TL:  // Left bumper (LB) - Decrease height
                    if (ev.value == 1) heightAdjustRequest_.store(-1);
                    break;
                case BTN_TR:  // Right bumper (RB) - Next gait / Increase height
                    if (ev.value == 1) gaitSwitchPressed_.store(true);
                    break;
            }
        }
        // Handle analog sticks
            ...
            switch (ev.code) {
                ...
                case ABS_Z:  // Left trigger
                    lt_.store(normalized > 0 ? normalized : 0);
                    break;
                case ABS_RZ:  // Right trigger
                    rt_.store(normalized > 0 ? normalized : 0);
                    break;
            }
        }
    }
...
int XboxController::getModeChangeRequest() {
    return modeChangeRequest_.exchange(-1);
}
...
bool XboxController::wasBackPressed() {
    return backPressed_.exchange(false);
}

bool XboxController::wasGaitSwitchPressed() {
    return gaitSwitchPressed_.exchange(false);
}

int XboxController::getHeightAdjustRequest() {
    return heightAdjustRequest_.exchange(0);
}
