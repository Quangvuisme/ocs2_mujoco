#ifndef ANYMAL_WALKING_XBOX_CONTROLLER_H
#define ANYMAL_WALKING_XBOX_CONTROLLER_H
...
/**
 * @brief Xbox controller input for ANYmal walking control
 * 
 * Joystick mapping:
 * Standing mode:
 ...
 * 
 * Walking mode:
 * - Left stick Y (ly): Forward/backward velocity
 * - Left stick X (lx): Left/right velocity
 * - Right stick X (rx): Yaw rate
 * - Right stick Y (ry): Height adjustment
 * 
 * Buttons:
 * - A: Switch to PASSIVE mode
 * - B: Switch to PD_CONTROL mode  
 * - X: Switch to STANDING mode (MPC)
 * - Y: Switch to WALKING mode (MPC + Gait)
 * - Start: Toggle MPC on/off
 * - Back: Emergency stop
 * - LB: Decrease height preset / previous gait
 * - RB: Increase height preset / next gait
 */
struct XboxInput {
...
public:
    ...
    // Get mode change request (-1 = no change, 0=PASSIVE, 1=PD_CONTROL, 2=STANDING, 3=WALKING)
    ...
    // Check if gait switch requested (RB button in walking mode)
...
#endif  // ANYMAL_WALKING_XBOX_CONTROLLER_H
