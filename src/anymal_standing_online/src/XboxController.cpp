...
void XboxController::runThread() {
    ...
    while (running_) {
        ...
        if (ev.type == EV_KEY) {
            switch (ev.code) {
                case BTN_A:  // A button - STANDING mode
                    if (ev.value == 1) modeChangeRequest_.store(1);
                    break;
                case BTN_B:  // B button - PASSIVE mode
                    if (ev.value == 1) modeChangeRequest_.store(0);
                    break;
                case BTN_X:  // X button - RECOVERY mode
                    if (ev.value == 1) modeChangeRequest_.store(2);
                    break;
                case BTN_Y:  // Y button - Reset pose
                    if (ev.value == 1) resetPressed_.store(true);
                    break;
                ...
                case BTN_TL:  // Left bumper (LB) - Decrease height
                    ...
                    break;
                case BTN_TR:  // Right bumper (RB) - Increase height
                    if (ev.value == 1) heightAdjustRequest_.store(1);
                    break;
            }
        }
...
bool XboxController::wasResetPressed() {
    return resetPressed_.exchange(false);
}
