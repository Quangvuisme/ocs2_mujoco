#include "anymal_walking_online/gait/LegLogic.h"

#include "anymal_walking_online/gait/MotionPhaseDefinition.h"
...
namespace {
    ...
    bool hasStartTime(const anymal_walking_online::ContactTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const anymal_walking_online::ContactTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool hasStartTime(const anymal_walking_online::SwingTiming &timing) {
        return !std::isnan(timing.start);
    }

    bool hasEndTime(const anymal_walking_online::SwingTiming &timing) {
        return !std::isnan(timing.end);
    }

    bool startsWithSwingPhase(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithContactPhase(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return !startsWithSwingPhase(timings);
    }

    bool endsWithSwingPhase(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithContactPhase(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return !endsWithSwingPhase(timings);
    }

    bool startsWithContactPhase(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return timings.empty() || hasStartTime(timings.front());
    }

    bool startsWithSwingPhase(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return !startsWithContactPhase(timings);
    }

    bool endsWithContactPhase(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return timings.empty() || hasEndTime(timings.back());
    }

    bool endsWithSwingPhase(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return !endsWithContactPhase(timings);
    }

    bool touchesDownAtLeastOnce(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const anymal_walking_online::ContactTiming &timing) {
            return hasStartTime(timing);
        });
    }

    bool liftsOffAtLeastOnce(const std::vector<anymal_walking_online::ContactTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool touchesDownAtLeastOnce(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return !timings.empty() && hasEndTime(timings.front());
    }

    bool liftsOffAtLeastOnce(const std::vector<anymal_walking_online::SwingTiming> &timings) {
        return std::any_of(timings.begin(), timings.end(), [](const anymal_walking_online::SwingTiming &timing) {
            return hasStartTime(timing);
        });
    }
} // anonymous namespace
...
namespace anymal_walking_online {
    feet_array_t<LegPhase> getContactPhasePerLeg(ocs2::scalar_t time, const ocs2::ModeSchedule &modeSchedule) {
        ...
        for (size_t leg = 0; leg < contactPhasePerLeg.size(); ++leg) {
            if (contactTimingsPerLeg[leg].empty()) {
                ...
                contactPhasePerLeg[leg].duration = std::numeric_limits<ocs2::scalar_t>::quiet_NaN();
                ...
                contactPhasePerLeg[leg].phase = 1.0;
                contactPhasePerLeg[leg].duration = std::numeric_limits<ocs2::scalar_t>::infinity();
    ...
    feet_array_t<LegPhase> getSwingPhasePerLeg(ocs2::scalar_t time, const ocs2::ModeSchedule &modeSchedule) {
        ...
        for (size_t leg = 0; leg < swingPhasePerLeg.size(); ++leg) {
            if (swingTimingsPerLeg[leg].empty()) {
                ...
                swingPhasePerLeg[leg].duration = std::numeric_limits<ocs2::scalar_t>::quiet_NaN();
                ...
                swingPhasePerLeg[leg].phase = 1.0;
                swingPhasePerLeg[leg].duration = std::numeric_limits<ocs2::scalar_t>::infinity();
                ...
                if (it == swingTimingsPerLeg[leg].end()) {
                    ...
                    swingPhasePerLeg[leg].phase = static_cast<ocs2::scalar_t>(-1.0);
                    swingPhasePerLeg[leg].duration = std::numeric_limits<ocs2::scalar_t>::quiet_NaN();
                } else {
    ...
    std::vector<ContactTiming> extractContactTimings(const std::vector<ocs2::scalar_t> &eventTimes,
        ...
        while (currentPhase < numPhases) {
            ...
            const ocs2::scalar_t startTime = (currentPhase == 0)
                                           ? std::numeric_limits<ocs2::scalar_t>::quiet_NaN()
            ...
            const ocs2::scalar_t endTime = (currentPhase + 1 >= numPhases)
                                         ? std::numeric_limits<ocs2::scalar_t>::quiet_NaN()
